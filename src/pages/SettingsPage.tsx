...
// Recurring Payment Settings Modal
interface RecurringPaymentSettingsModalProps {
...
const RecurringPaymentSettingsModal = ({
  recurringPayment, accounts, paymentMethods, onSave, onClose, onDelete,
}: RecurringPaymentSettingsModalProps) => {
  ...
  const [accountId, setAccountId] = useState(recurringPayment?.accountId || (accounts.length > 0 ? accounts[0].id : ''));
  const [pmId, setPmId] = useState<string | undefined>(recurringPayment?.paymentMethodId);
  ...
  const linkedPaymentMethods = pmId
    ? paymentMethods.filter((pm) => pm.id === pmId)
    : accountId
      ? paymentMethods.filter((pm) => pm.linkedAccountId === accountId)
      : paymentMethods;
          ...
          <div className="space-y-4 sm:space-y-5">
            ...
            <div>
              ...
              <select
                ...
                className="w-full border border-gray-300 dark:border-gray-600 dark:bg-slate-700 dark:text-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-600"
                required
              >
            ...
            <div>
              <label className="block text-xs sm:text-sm font-semibold text-gray-900 dark:text-gray-200 mb-2">支払い元（口座）</label>
              <select
                value={accountId}
            ...
            {linkedPaymentMethods.length > 0 && (
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-gray-900 dark:text-gray-200 mb-2">支払い手段（オプション）</label>
                <select
                  value={pmId || ''}
                  onChange={(e) => setPmId(e.target.value || undefined)}
                  className="w-full border border-gray-300 dark:border-gray-600 dark:bg-slate-700 dark:text-gray-100 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-600"
                >
            ...
            <div>
              <label className="block text-xs sm:text-sm font-semibold text-gray-900 dark:text-gray-200 mb-2">実行日</label>
              <select
                value={dayOfMonth}
            ...
            {frequency === 'yearly' && (
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-gray-900 dark:text-gray-200 mb-2">実行月</label>
                <select
                  value={monthOfYear}
              ...
              <button
                ...
                className={`w-full py-2 px-3 rounded-lg font-medium text-sm transition-colors ${
                  isActive
                    ...
                    : 'bg-gray-50 dark:bg-slate-700 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-600'
                }`}
              >
                {isActive ? '✓ 有効' : '✗ 無効'}
              </button>
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 p-3 sm:p-4 space-y-2">
          ...
          <div className="flex gap-3">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-2 px-3 sm:px-4 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-200 font-medium text-sm hover:bg-gray-100 dark:hover:bg-slate-700"
            >
              キャンセル
            </button>
            <button
              type="submit"
              className="flex-1 py-2 px-3 sm:px-4 rounded-lg text-white font-medium text-sm transition-colors hover:opacity-90"
              style={{ backgroundColor: 'var(--theme-primary)' }}
            >
              保存
            </button>
