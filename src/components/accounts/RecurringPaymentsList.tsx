...
export const RecurringPaymentsList = ({ items, onAdd, onEdit, onToggle, getCategory }: RecurringPaymentsListProps) => {
  ...
  return (
 <div className="mt-3 pt-3 ">  <div className="flex justify-between items-center mb-1.5">         <button
 ...
 <button onClick={onAdd} className="text-gray-700 hover:text-gray-800 dark:text-gray-600">           <Plus size={16} />
        </button>
      </div>
      {isOpen && (
        <>
 ...
 <div className="space-y-2">               {items.map((rp) => {
                const category = getCategory(rp.categoryId);
                const freqLabel = rp.frequency === 'monthly'
                  ? `毎月${rp.dayOfMonth}日`
                  : `毎年${rp.monthOfYear}月${rp.dayOfMonth}日`;
                return (
 <div key={rp.id} className={`flex items-center justify-between text-sm ${rp.isActive ? '' : 'opacity-40'}`}>  <div className="flex items-center gap-2 min-w-0 flex-1">  <button onClick={() => onToggle(rp)} className="flex-shrink-0">                         {rp.isActive
 ? <ToggleRight size={18} className="text-gray-600" />  : <ToggleLeft size={18} className="text-gray-300 dark:text-gray-600" />                         }
                      </button>
 <button onClick={() => onEdit(rp)} className="flex items-center gap-2 min-w-0 flex-1 text-left hover:opacity-70 transition-opacity">                         <div
