...
import { Filter, Calendar, DollarSign, Tag, CreditCard, Wallet, ArrowUpDown, X, Search, RotateCcw, ArrowUp, ArrowDown, Layers } from 'lucide-react';
...
type FilterType = 'type' | 'date' | 'category' | 'account' | 'payment' | 'sort' | 'search';
                ...
                {filterMenuItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* 各フィルターのアクティブ数バッジ */}
                      {item.activeCount > 0 && (
                        <span className="absolute -top-1.5 -right-1.5 bg-gray-800 text-white text-sm rounded-full w-5 h-5 flex items-center justify-center font-bold border-2 border-white dark:border-slate-800">
                          {item.activeCount}
                        </span>
                      )}
                    </button>
