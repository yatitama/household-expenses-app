...
月別の収支を可視化するホーム画面。成長と達成感を実感できるUI/UXが特徴。

### 機能

#### 成長指標セクション（新機能）
ページ最上部に配置され、ユーザーの経済的な成長を可視化します：

- **先月比カード**（GrowthHeader）
  - 先月との収支差分を金額・パーセンテージで表示
  - ポジティブな変化は緑色でハイライト、スパークルアイコン付き
  - 条件付きメッセージ：「先月より頑張れた！」など

- **推移チャート**（TrendChart）
  - 3ヶ月/6ヶ月/1年の時間範囲を切り替え可能（デフォルト: 6ヶ月）
  - Rechartsを使用した二重軸エリアチャート
  - 純資産と月間黒字を同時に可視化
  - インタラクティブなホバーツールチップで詳細表示

- **月別比較カード**（MonthComparisonCards）
  - 現在月と前月を並べて比較表示
  - 収入、支出、純資産（残高合計）、貯金額を表示
  - 改善指標に上昇/下降矢印と成長率を表示

- **達成バッジ**（AchievementBadges）
  - 達成された4つのマイルストーン：
    - 貯金目標達成 🎉
    - 支出削減 💪（前月比削減時）
    - 月の黒字化 📈（収入 > 支出時）
    - 黒字連続 ✅（3ヶ月以上連続）
  - 横スクロール対応（モバイル）、グリッド表示（デスクトップ）

#### その他セクション（既存）
- **月セレクタ**: 前月/次月のナビゲーション（スワイプ対応）
- **収入セクション**: 月内の収入取引をカテゴリ別にグリッド表示
- **支出セクション**: カテゴリ別、支払い手段別、メンバー別の3つのグループ切替で表示
  - **カテゴリグリッド**: 予算設定時は予算額と実績をプログレスバーで視覚化
  - **支払い手段別グリッド**: カード別に支出集計。予算設定時は進捗バーを表示
  - **メンバー別グリッド**: メンバー別に支出集計。予算設定時は進捗バーを表示
- **定期取引セクション**: 当月に発生する定期取引の一覧
- **貯金目標セクション**（SavingsProgressTimeline追加）
  - 主要貯金目標の進捗を優先表示
  - 目標達成までの月数と月額を表示
  - 達成状況に応じた色分けプログレスバー
  - 従来の貯金目標グリッドも表示（月別調整可能）
- **フッター**: 月の収入合計 - 支出合計 の差額を常時表示

### 使用コンポーネント
- `GrowthHeader` — 先月比較表示
- `TrendChart` — 推移チャート（Recharts）
- `MonthComparisonCards` — 月別比較
- `AchievementBadges` — 達成バッジ表示
- `SavingsProgressTimeline` — 貯金目標の進捗表示
- `CardGridSection` — 支出・収入のカテゴリ別グリッド
